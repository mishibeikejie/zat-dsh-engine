	...
	return class ZatMarketGateway extends _classSuper {
			...
			return defineTool({
				...
				execute: async (args) => {
					...
					if (r.status !== 200) return {
						items: [],
						notice: `搜索失败(${r.status})${r.error ? ":" + r.error.slice(0, 120) : ""}。稍后再试,或换个说法。`
					};
					let raw = [];
					try {
		...
		*/
		async analyzeCandidateConflicts(owner, repo, subdir) {
			...
			for (const sec of scanSecurity(f.hostText, "宿主代码")) if (sec.level === "error") block.push(`安全:${sec.title}`);
			else warn.push(sec.title);
			for (const sec of scanSecurity(f.clientText, "界面代码")) if (sec.level === "error") block.push(`安全:${sec.title}`);
			else warn.push(sec.title);
			return {
				block,
				warn
		...
		*/
		...
		async addSpec(owner, repo, subdir, taskId, preAnalysis) {
			...
			if (pnpmResult.outcome.exitCode !== 0) {
				...
				if (/pnpm[^\n]*(?:不是内部或外部命令|无法识别|command not found|is not recognized)|corepack[^\n]*(?:不是内部或外部命令|无法识别|command not found|is not recognized)/i.test(errText)) return {
					ok: false,
					packageName: null,
					message: "安装失败:本机没有可用的 pnpm。pnpm 是 dsh 插件系统的必备组件,请先安装 Node.js(自带 corepack,运行 corepack enable)或安装 pnpm 后重试。"
				};
...
//#endregion
export { ZatMarketGateway, ZatMarketGateway as default, compareVersions, scanSecurity };
