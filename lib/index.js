		...
		buildFindPluginTool() {
			return defineTool({
				...
				execute: async (args) => {
					...
					return {
						items,
						notice: items.length === 0 ? "没有找到匹配的插件,换个说法试试" : `找到 ${items.length} 个候选,已对可安装的候选做装前体检。规则:带 ❌/[error] 硬伤的候选装了大概率用不了,别推荐安装,把问题如实告诉用户;带 ⚠️/[warn] 的提醒用户注意;✅ 的可以放心推荐。装完仍建议在插件市场点「一键检测」查与已装插件的冲突。` + (risky > 0 ? ` 本批有 ${risky} 个候选存在硬伤。` : "")
					};
				}
			});
		}
