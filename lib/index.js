...
let ZatMarketGateway = (() => {
	...
	return class ZatMarketGateway extends _classSuper {
		...
		*/
		async checkMarketConflict(owner, repo) {
			...
			try {
				...
				const conflicts = [];
				for (const rec of Object.values(inst)) if (KNOWN_MARKET_REPOS[(rec.owner + "/" + rec.repo).toLowerCase()] !== void 0 || Object.values(KNOWN_MARKET_REPOS).includes(rec.name) || isMarketishName(rec.name)) conflicts.push(rec.name);
		...
		async fetchCandidateTexts(owner, repo, subdir) {
			...
			const candidates = [meta.main];
			for (const v of Object.values(meta.exports || {})) if (typeof v === "string") candidates.push(v);
			else if (v && typeof v === "object" && typeof v.default === "string") candidates.push(v.default);
			candidates.push("lib/host.js", "lib/index.js", "dist/index.js", "lib/client.js", "dist/client.js");
			let hostText = "";
			let clientText = "";
		...
		async anyInstalledMarketish() {
			try {
				const p = await this.readProfile();
				const inst = this.installedMap(p);
				for (const rec of Object.values(inst)) if (KNOWN_MARKET_REPOS[(rec.owner + "/" + rec.repo).toLowerCase()] !== void 0 || Object.values(KNOWN_MARKET_REPOS).includes(rec.name) || isMarketishName(rec.name) || await this.scanLocalMarketish(rec.name)) return rec.name;
			} catch {}
			return null;
		}
		/**
		...
		*/
		async pnpmShell(command, dir, onProgress) {
			...
			if (IS_WIN) {
				...
				if (this.directDown) full = proxySetup + mirrorWin + command;
				else full = proxySetup + command + "; if ($LASTEXITCODE -ne 0) { $env:GIT_CONFIG_COUNT=1; $env:GIT_CONFIG_KEY_0='url.https://gh-proxy.com/https://github.com/.insteadOf'; $env:GIT_CONFIG_VALUE_0='https://github.com/';" + command + " }";
		...
		*/
		...
		async addSpec(owner, repo, subdir, taskId) {
			...
			try {
				...
				return {
					...
					taskId: this.launchTask(async (id) => {
						...
						const res = await this.addSpec(o, r, s || void 0, id);
						return res.ok ? {
							ok: true,
							packageName: res.packageName,
							message: `已安装 github:${o}/${r}${s ? `#path:${s}` : ""} — 重启 dsh 后生效${res.warning ? "。风险提示:" + res.warning : ""}`
						} : {
							ok: false,
							packageName: null,
							message: res.message
				...
				return {
					...
					taskId: this.launchTask(async (id) => {
						...
						return res.ok ? {
							ok: true,
							version,
							message: `已更新 github:${o}/${r}${s ? `#path:${s}` : ""} 到 v${version || "?"} — 重启 dsh 后生效${res.warning ? "。风险提示:" + res.warning : ""}`
						} : {
							ok: false,
							message: res.message
						};
			...
			try {
				...
				const worker = async () => {
					while (next < unique.length) {
						...
						try {
							const it = JSON.parse(repoRes.body);
							const cachedZh = this.zhCache.get(fullName.toLowerCase());
							const zhIntro = cachedZh && Date.now() - cachedZh.at < ZH_TTL ? cachedZh.zh : "";
							const item = {
								fullName,
		...
		*/
		async healthCheck() {
			...
			try {
				...
				const inst = this.installedMap(p);
				const markets = [];
				for (const rec of Object.values(inst)) if (KNOWN_MARKET_REPOS[(rec.owner + "/" + rec.repo).toLowerCase()] !== void 0 || Object.values(KNOWN_MARKET_REPOS).includes(rec.name) || isMarketishName(rec.name) || await this.scanLocalMarketish(rec.name)) markets.push(rec.name);
