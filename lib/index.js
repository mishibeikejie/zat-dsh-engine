...
let ZatMarketGateway = (() => {
	...
	return class ZatMarketGateway extends _classSuper {
		...
		constructor(ctx) {
			...
			this.fs = this.ctx.get("fs");
			this.llm = this.ctx.get("llm");
		}
		async runPowershell(command, cwd) {
		...
		*/
		...
		async getProfileName() {
			...
			try {
				const v = ((await this.runPowershell("$e=[Environment]::GetEnvironmentVariable(\"DSH_PROFILE\",\"Process\"); Write-Output $e")).stdout || "").trim();
				if (v) {
					this.profileNameValue = v;
					return v;
				}
			} catch {}
			const dir = await this.getHome() + "\\profiles";
			let names = [];
			try {
				const dirTarget = await this.fs.resolve(dir);
				names = (await this.fs.listDir(dirTarget)).map((e) => e.name);
			} catch {
				names = [];
			}
		...
		async curlGet(url) {
			...
			const handle = this.subprocess.spawn({
				argv: [
					...
					"\n%{http_code}",
					"-H",
					"User-Agent: zat-dsh-engine/0.1.0",
					url
				],
				cwd: "C:\\",
				stdio: {
		...
		async readProfile() {
			...
			return JSON.parse(await this.fs.readText(target));
		}
		...
		async remoteVersion(owner, repo) {
		...
		async localVersion(name) {
			try {
				...
				return JSON.parse(await this.fs.readText(t)).version || null;
			} catch {
				return null;
			}
		}
		async addSpec(owner, repo) {
			...
			for (const name of deps) {
				...
				if (!String((after.dependencies || {})[name] || "").toLowerCase().includes(owner.toLowerCase() + "/" + repo.toLowerCase())) continue;
				try {
					const t = await this.fs.resolve(dir + "\\node_modules\\" + name + "\\package.json");
					if (JSON.parse(await this.fs.readText(t)).dsh?.bundle?.patch) {
						bundles.push(name);
						added = name;
					}
				} catch {}
		...
		async list(page, sort, q, category) {
			try {
				...
				if (r.status !== 200) return {
					ok: false,
					message: `GitHub API ${r.status} — direct and mirror both failed, please check your network`
				};
				let json = null;
				try {
		...
		async installed() {
			try {
				const p = await this.readProfile();
				const inst = this.installedMap(p);
				const entries = Object.keys(inst).map((key) => ({
					key,
					name: inst[key].name,
					spec: inst[key].spec
				}));
		...
		async install(owner, repo) {
			try {
				const o = String(owner || "");
				const r = String(repo || "");
				if ((await this.ghGet(`https://raw.githubusercontent.com/${o}/${r}/HEAD/package.json`)).status !== 200) return {
					ok: false,
					message: `⚠ ${o}/${r} 是多插件仓库(根目录没有 package.json),无法一键安装。请到该仓库的 GitHub 页面查看各子插件的安装方式。`
				};
				const res = await this.addSpec(o, r);
				return res.ok ? {
					ok: true,
					packageName: res.packageName,
					message: `installed github:${o}/${r} — restart dsh to activate`
				} : {
					ok: false,
					message: res.message
				};
		...
		async update(owner, repo) {
			try {
				...
				return res.ok ? {
					ok: true,
					version,
					message: `updated github:${o}/${r} to v${version || "?"} — restart dsh to activate`
				} : {
					ok: false,
					message: res.message
				};
