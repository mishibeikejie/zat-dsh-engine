		...
		*/
		...
		async addSpec(owner, repo, subdir, taskId, preAnalysis) {
			...
			if (pnpmResult.outcome.exitCode !== 0) {
				...
				return {
					ok: false,
					packageName: null,
					message: `安装失败:${/UND_ERR|ECONN|ETIMEDOUT|Failed to connect|ENOTFOUND|network|fetch/i.test(errText) ? "连不上 GitHub/npm(网络问题)" : lastLine.slice(0, 120) || "未知原因"}。已自动回滚,换个网络或稍后重试。`
				};
			}
			if (taskId) {
		...
		async list(page, sort, q, category) {
			try {
				...
				if (r.status !== 200) return {
					ok: false,
					message: `搜索失败(${r.status})。${r.status === 400 || r.status === 422 ? "搜索词无效,换个说法试试。" : "连不上 GitHub,请开代理或稍后重试。"}`
				};
				let json = null;
				try {
