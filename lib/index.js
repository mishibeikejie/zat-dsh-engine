	...
	return class ZatMarketGateway extends _classSuper {
		...
		async checkMarketConflict(owner, repo) {
			...
			try {
				...
				for (const rec of Object.values(inst)) {
					...
					if (KNOWN_MARKET_REPOS[(rec.owner + "/" + rec.repo).toLowerCase()] !== void 0 || KNOWN_MARKET_REPOS[candidateRepo] !== void 0 && Object.values(KNOWN_MARKET_REPOS).includes(rec.name)) conflicts.push(rec.name);
				}
				if (conflicts.length > 0) return `已拦截安装:你的 profile 里已经装了市场/管理器类插件 ${conflicts.join("、")}。两个市场类插件同时存在会互相覆盖设置页、注册冲突,可能让 dsh 直接起不来(已经有人踩过这个坑)。想装这个,请先在市场里把原来的卸载掉;装完重启后如果再换回来,同样先卸载再装。`;
				return null;
			} catch {
				return null;
			}
		...
		*/
		async analyzeCandidateConflicts(owner, repo, subdir) {
			...
			for (const d of Object.keys(meta.dependencies || {})) if (d.startsWith("@deepseek-ai/")) block.push(`把官方包 ${d} 写进了 dependencies(应改用 peerDependencies),会装出第二份拷贝并劫持官方 loader 行`);
		...
		*/
		async httpGet(url) {
			...
			if (curl) {
				const handle = this.subprocess.spawn({
					argv: [
						...
						"\n%{http_code}",
						"-H",
						"User-Agent: zat-dsh-engine/0.2.0",
						url
					],
					cwd: this.shellCwd(),
					stdio: {
			...
			return {
				status: 0,
				body: "",
				error: "no curl or wget available on this system"
			};
		}
		async ghGet(url) {
			let lastError = "";
		...
		async addSpec(owner, repo, subdir) {
			...
			const analysis = await this.analyzeCandidateConflicts(o, repoName, s || void 0);
			if (analysis.block.length > 0) return {
				ok: false,
				packageName: null,
				message: "安装前检查发现硬冲突,已拦截:\n- " + analysis.block.join("\n- ") + `\n确定仍要安装,请用官方命令强制安装(风险自负): dsh plugin --profile <你的profile> add ${spec}`
			};
			...
			if (pnpmResult.outcome.exitCode !== 0) {
				...
				if (errText.includes("PREPARE_NOT_ALLOWED") || errText.includes("allowBuilds") || errText.includes("build script")) return {
					ok: false,
					packageName: null,
				...
				return {
					ok: false,
					packageName: null,
					message: (errText.slice(0, 2e3) || "pnpm failed") + " — profile 配置已自动回滚到安装前状态"
				};
			}
		...
		async list(page, sort, q, category) {
			try {
				...
				if (r.status !== 200) return {
					ok: false,
					message: `GitHub 请求失败(${r.status})${r.error ? " — " + r.error : ""}。直连与镜像均未成功:请确认网络可用;使用 VPN 时请开启系统代理模式,或安装 curl(Windows 自带)。`
				};
				let json = null;
				try {
		...
		async detail(owner, repo) {
			try {
				...
				const isMonorepo = (await this.ghGet(`https://raw.githubusercontent.com/${o}/${r}/HEAD/package.json`)).status !== 200;
				let notPlugin = false;
				if (isMonorepo) {
		...
		async selfupdate(doUpdate) {
			...
			if (!doUpdate) {
				...
				if (!remote || remote === SELF_VERSION) return {
					ok: true,
					hasUpdate: false,
			...
			if (r.outcome.exitCode !== 0) return {
				ok: false,
				message: (r.stderr || r.stdout || "pnpm failed").slice(0, 2e3)
			};
			return {
				ok: true,
				message: "updated to v" + await this.remoteVersion(owner, repo) + " — restart dsh to activate"
			};
		}
		...
		async install(owner, repo, subdir) {
			try {
				...
				const res = await this.addSpec(o, r, s || void 0);
				return res.ok ? {
					ok: true,
					packageName: res.packageName,
					message: `installed github:${o}/${r}${s ? `#path:${s}` : ""} — restart dsh to activate${res.warning ? ". 风险提示:" + res.warning : ""}`
				} : {
					ok: false,
					message: res.message
				};
		...
		async update(owner, repo, subdir) {
			try {
				...
				return res.ok ? {
					ok: true,
					version,
					message: `updated github:${o}/${r}${s ? `#path:${s}` : ""} to v${version || "?"} — restart dsh to activate${res.warning ? ". 风险提示:" + res.warning : ""}`
				} : {
					ok: false,
					message: res.message
				};
		...
		async setEnabled(name, enabled) {
			try {
				...
				return {
					ok: true,
					enabled,
					message: enabled ? `${n} 已启用 — 重启 dsh 后生效` : `${n} 已停用 — 重启 dsh 后生效`
				};
			} catch (err) {
				return {
					ok: false,
		...
		async healthCheck() {
			...
			try {
				...
				for (const name of deps) try {
					...
					for (const pd of Object.keys(meta.peerDependencies || {})) if (!(deps.includes(pd) || await this.installedVersionOf(pd) !== null)) issues.push({
						level: "warn",
						title: `${name} 需要的 peer 依赖 ${pd} 未安装`,
						detail: "profile 关闭了自动安装 peer;这个依赖缺失时插件运行时可能报错。手动安装它或反馈给插件作者。"
					});
					if (!bundles.includes(name) && !name.startsWith("@deepseek-ai/")) issues.push({
						level: "info",
						title: `${name} 已停用`,
						detail: "已安装但不在启用名单,重启后不会加载。可在市场卡片上点「启用」。"
					});
				} catch {
					issues.push({
						level: "warn",
						title: `找不到 ${name} 的包文件`,
						detail: "依赖名单里有它,但 node_modules 里没有。可能安装未完成,重装一次即可。"
					});
				}
				const idHolders = /* @__PURE__ */ new Map();
				for (const s of scanned) for (const id of s.patchIds) {
					const holder = idHolders.get(id);
					if (holder && holder !== s.name) issues.push({
						level: "error",
						title: `挂载行 id "${id}" 重复`,
						detail: `${holder} 和 ${s.name} 都声明了这个行 id,加载时会互相冲突,建议二选一。`
					});
					else if (!holder) idHolders.set(id, s.name);
				}
				const declared = /* @__PURE__ */ new Map();
				for (const s of scanned) for (const [dep, range] of [...Object.entries(s.meta.dependencies || {}), ...Object.entries(s.meta.peerDependencies || {})]) {
					if (dep.startsWith("@deepseek-ai/")) continue;
				...
				const inst = this.installedMap(p);
				const markets = [];
				for (const rec of Object.values(inst)) if (KNOWN_MARKET_REPOS[(rec.owner + "/" + rec.repo).toLowerCase()] !== void 0 || Object.values(KNOWN_MARKET_REPOS).includes(rec.name)) markets.push(rec.name);
...
//#endregion
export { ZatMarketGateway, ZatMarketGateway as default };
