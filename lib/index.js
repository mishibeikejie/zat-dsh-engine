		...
		*/
		async healthCheck() {
			...
			try {
				...
				if (!pnpmOk) issues.push({
					level: "error",
					title: "没装 pnpm",
					detail: "装/更新插件都靠它。解决:终端里跑一条 corepack enable(或 npm i -g pnpm)。"
				});
		...
		async repair() {
			...
			try {
				...
				return {
					...
					fixed,
					remaining,
					message: `修复 ${fixed.length} 项。` + (remaining.length ? `还有 ${remaining.length} 项修不了:${remaining.join(";")}` : "")
				};
			} catch (err) {
				return {
					ok: false,
