		...
		*/
		async pnpmShell(command, dir, onProgress) {
			...
			if (IS_WIN) {
				...
				full = proxySetup + registryWin + pnpmSetup + mirrorWin + run + "; if ($LASTEXITCODE -ne 0) { Remove-Item Env:HTTPS_PROXY,Env:HTTP_PROXY,Env:ALL_PROXY,Env:https_proxy,Env:http_proxy,Env:all_proxy,Env:NO_PROXY,Env:no_proxy -ErrorAction SilentlyContinue;" + run + "; if ($LASTEXITCODE -ne 0) { Remove-Item Env:GIT_CONFIG_COUNT,Env:GIT_CONFIG_KEY_0,Env:GIT_CONFIG_VALUE_0 -ErrorAction SilentlyContinue;" + run + " } }";
			} else full = "export PATH=\"" + dirname(process.execPath) + ":$PATH\"; export npm_config_registry='https://registry.npmmirror.com';export GIT_CONFIG_COUNT=1; export GIT_CONFIG_KEY_0='url.https://gh-proxy.com/https://github.com/.insteadOf'; export GIT_CONFIG_VALUE_0='https://github.com/';" + command + " || { unset GIT_CONFIG_COUNT GIT_CONFIG_KEY_0 GIT_CONFIG_VALUE_0 2>/dev/null;" + command + " }";
