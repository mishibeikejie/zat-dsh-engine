				...
				execute: async (args) => {
					...
					const items = picked.map((it, i) => {
						...
						return {
							...
							kind,
							installable,
							install: kind === "plugin" ? `dsh plugin --profile web add github:${it.fullName}` : kind === "client" ? "在插件市场点「安装」一键装(主题/界面插件,刷新页面生效)" : kind === "skill" ? "在插件市场点「安装」一键装(技能包,装进 ~/.dsh/skills 立即生效)" : "",
							url: it.url,
							health: healths[i]
						};
					});
