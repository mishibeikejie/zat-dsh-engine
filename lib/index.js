...
*/
function listHeader(raw) {
	if (!raw || typeof raw !== "object") return void 0;
	const r = raw;
	const h = r.header && typeof r.header === "object" ? r.header : r;
	if (typeof h.id !== "string" || !h.id) return void 0;
	return h;
}
/** 投影缓存记录 `{ identity, rows }`;兼容带 `{ version, record }` 外壳的原始分片。 */
function checkpointRecordOf(raw) {
	if (!raw || typeof raw !== "object") return void 0;
	const r = raw;
	return r.record && typeof r.record === "object" ? r.record : r;
}
/**
...
let ZatMarketGateway = (() => {
	...
	return class ZatMarketGateway extends _classSuper {
		...
		* 非 Windows 平台原样透传。
		*/
		async winHiddenSpawn(argv, opts = {}) {
			const cwd = opts.cwd || this.shellCwd();
			const stdio = {
				stdin: opts.stdin !== void 0 ? { data: opts.stdin } : "ignore",
				stdout: { maxBytes: opts.stdoutMax || 16777216 },
				stderr: { maxBytes: opts.stderrMax || 1048576 }
			};
			if (!IS_WIN) return this.subprocess.spawn({
				argv,
				cwd,
				stdio,
				graceMs: opts.graceMs || 6e4
			});
			return this.nodeSpawnHidden(argv, opts);
		}
		/** 启动器内置工具链目录(存在才返回,拼进子进程 PATH):%TEMP%\zat-tools 与 %TEMP%\zat-tools\git\cmd。 */
		launcherToolPathPrefix() {
			const sep = IS_WIN ? ";" : ":";
			const parts = [];
			if (process.env.TEMP) {
		...
		nodeSpawnHidden(argv, opts = {}) {
			const childEnv = opts.env ? { ...opts.env } : { ...process.env };
			if (IS_WIN) childEnv.PATH = this.launcherToolPathPrefix() + dirname(process.execPath) + ";" + (childEnv.PATH || "");
			...
			let stdoutLen = 0;
			let stderrLen = 0;
			const stdoutMax = opts.stdoutMax || 16777216;
			const stderrMax = opts.stderrMax || 1048576;
			child.stdout?.on("data", (d) => {
				const room = stdoutMax - stdoutLen;
				if (room > 0) {
					const slice = d.subarray(0, room);
			...
			const reader = (chunks) => ({ readFrom: (offset) => ({ text: Buffer.concat(chunks).toString("utf8").slice(offset) }) });
			return {
				done,
				collected: {
					stdout: reader(stdoutChunks),
					stderr: reader(stderrChunks)
				}
			};
		}
		...
		*/
		...
		async ensureAllowBuilds(name) {
			try {
				...
				let allow = {};
				if (ws.allowBuilds && typeof ws.allowBuilds === "object" && !Array.isArray(ws.allowBuilds)) allow = ws.allowBuilds;
				else if (Array.isArray(ws.allowBuilds)) {
					for (const item of ws.allowBuilds) if (typeof item === "string" && item.trim()) allow[item.trim()] = true;
				}
		...
		*/
		npmInfoCache = /* @__PURE__ */ new Map();
		async npmLatest(name) {
			const key = name.toLowerCase();
			const hit = this.npmInfoCache.get(key);
			if (hit !== void 0 && Date.now() - hit.at < 18e5) return {
				version: hit.version,
			...
			try {
				...
				if (reg.status === 200) {
					...
					const v = latest ? j.versions?.[latest] : void 0;
					if (v && typeof v.version === "string") {
						const repo = v.repository;
						out = {
							version: v.version,
							repository: typeof repo === "string" ? repo : repo && typeof repo.url === "string" ? repo.url : void 0
						};
					}
				}
			} catch {
			...
			if (out) this.npmInfoCache.set(key, {
				version: out.version,
		...
		repoPointsToGithub(repoField, owner, repo) {
		...
		*/
		async sameSourceNpm(metaName, owner, repo) {
			...
			try {
				...
				if (missingEntries.length > 0) {
					if (hasBuildScript(meta.scripts)) checks.push({
						...
						if (same) checks.push({
							level: "warn",
							title: `GitHub 仓库只是源码(入口 ${missingEntries.join("、")} 未提交),npm 已有同源发布 ${same.name}@${same.version}`,
							detail: `正式发布在 npm:点「安装」会自动从 npm 官方源安装 ${same.name},效果等同官方 dsh plugin add ${same.name},装完即可加载。`
						});
						else checks.push({
							level: "error",
							title: `入口文件缺失:${missingEntries.join("、")}`,
							detail: "package.json 声明的入口在仓库里不存在,且 npm 上也没有同源发布——最常见的原因是构建产物(dist)没有提交到 git。装完 dsh 加载就会报错,插件等于用不了。"
						});
					}
				}
		...
		*/
		async pnpmShell(command, dir, onProgress) {
			...
			if (!argvBase) return {
				...
				stdout: "",
				stderr: IS_WIN ? "pnpm is not recognized as an internal or external command" : "pnpm: command not found"
			};
			...
			if (IS_WIN) {
				...
				if (proxy) {
					a1.HTTPS_PROXY = proxy;
					a1.HTTP_PROXY = proxy;
					a1.ALL_PROXY = proxy;
					a1.NO_PROXY = "localhost,127.0.0.1";
				}
			}
			...
			const a2 = { ...a1 };
			delete a2.HTTPS_PROXY;
			delete a2.HTTP_PROXY;
			delete a2.ALL_PROXY;
			delete a2.NO_PROXY;
			const a3 = { ...a2 };
			delete a3.GIT_CONFIG_COUNT;
			delete a3.GIT_CONFIG_KEY_0;
			delete a3.GIT_CONFIG_VALUE_0;
			let last = {
				outcome: { exitCode: 127 },
				stdout: "",
				stderr: ""
			...
			for (const env of [
				...
				const childEnvHandle = this.nodeSpawnHidden([...argvBase, ...args], {
					cwd: dir,
					graceMs: 6e5,
					onStdout: onProgress,
					env
		...
		*   pnpm 装哪就是哪,用户装在哪不关我们事;自带没有就明确失败,不悄悄用系统的。 */
		async resolvePnpmCommand() {
			const cands = [];
			if (process.env.PNPM_MJS) cands.push(process.env.PNPM_MJS);
			const zt = join(process.env.TEMP || "", "zat-tools");
			try {
				const verDirs = readdirSync(zt).filter((n) => /^pnpm-\d+\.\d+\.\d+$/.test(n) && existsSync(join(zt, n, "pnpm.exe"))).sort();
		...
		*/
		...
		async curlGet(url, proxy, maxTime) {
			...
			const handle = await this.winHiddenSpawn(argv, { graceMs: 6e4 });
			const outcome = await handle.done;
			let stdout = "";
			let stderr = "";
			if (handle.collected?.stdout) stdout = handle.collected.stdout.readFrom(0).text || "";
		...
		*/
		...
		async addSpec(owner, repo, subdir, taskId, preAnalysis) {
			...
			if (npmSource) {
				...
				if (npmResult.outcome.exitCode !== 0) for (let round = 0; round < 3 && npmResult.outcome.exitCode !== 0; round++) {
				...
				if (npmResult.outcome.exitCode !== 0) {
					...
					return {
						ok: false,
						packageName: null,
						message: `从 npm 安装 ${npmSource} 失败(已还原)。${(errText.trim().split(/\r?\n/).filter(Boolean).pop() || "").slice(-160)}`
					};
				}
			} else {
				...
				if (pnpmResult.outcome.exitCode !== 0) for (let round = 0; round < 3 && pnpmResult.outcome.exitCode !== 0; round++) {
				...
				if (pnpmResult.outcome.exitCode !== 0) {
					...
					const reason = gitMissing ? "本机没装 git,而 git 仓库插件必须用它来拉取" : gitish || networkish ? "拉取插件源码/依赖失败(网络、镜像或 git 认证问题,不是文件权限问题)" : fileBusy ? "文件被占用或没有写权限(多半是文件被别的程序锁住、目录只读,或需要管理员权限)" : lastLine.slice(0, 120) || "未知原因";
					const advice = gitMissing ? "解决:安装 git(winget install Git.Git 或从 git-scm.com 下载),装完再点安装。" : gitish || networkish ? "解决:开代理/VPN 或稍后重试;市场会自动依次换 gh-proxy/ghfast 镜像重试。" : fileBusy ? "解决:关掉占用它的程序(先停 DSH)、确认目录不是只读、必要时用管理员运行,再重试。" : "换个网络或稍后重试。";
			...
			for (const name of deps) {
				...
				const specVal = String((after.dependencies || {})[name] || "");
				if (npmSource) {
					if (name.toLowerCase() !== npmSource.toLowerCase()) continue;
				} else if (!specVal.toLowerCase().includes(o.toLowerCase() + "/" + repoName.toLowerCase())) continue;
				matched = true;
				matchedName = name;
				try {
					const meta = JSON.parse(readFileSync(join(dir, "node_modules", name, "package.json"), "utf8"));
		...
		async selfupdate(doUpdate, zhLocale) {
			...
			if (!doUpdate) {
				...
				if (compareVersions(remote, SELF_VERSION) <= 0) return {
					ok: true,
					hasUpdate: false,
					current: SELF_VERSION,
			...
			try {
				...
				return {
					...
					taskId: this.launchTask(async (id) => {
						...
						const installedLabel = analysis.npmName ? `npm 包 ${analysis.npmName}(github:${o}/${r} 的官方发布)` : `github:${o}/${r}${s ? `#path:${s}` : ""}`;
						return res.ok ? {
							ok: true,
							packageName: res.packageName,
							message: res.hotReload ? `已安装(主题/界面插件)— 刷新页面即可生效。${res.warning ? "。风险提示:" + res.warning : ""}` : `已安装 ${installedLabel} — 重启 dsh 后生效。${analysis.usage[0] || ""}${res.warning ? "。风险提示:" + res.warning : ""}`
						} : {
							ok: false,
							packageName: res.packageName,
							installedAsDisabled: res.installedAsDisabled === true,
			...
			try {
				...
				return {
					...
					taskId: this.launchTask(async (id) => {
						...
						const res = await this.addSpec(o, r, s || void 0, id, analysis);
						const version = analysis.npmName ? (await this.npmLatest(analysis.npmName))?.version || null : await this.remoteVersion(o, r, s || void 0);
						const updatedLabel = analysis.npmName ? `npm 包 ${analysis.npmName}(github:${o}/${r} 的官方发布)` : `github:${o}/${r}${s ? `#path:${s}` : ""}`;
						return res.ok ? {
							ok: true,
							version,
							message: `已更新 ${updatedLabel} 到 v${version || "?"} — 重启 dsh 后生效。${analysis.usage[0] || ""}${res.warning ? "。风险提示:" + res.warning : ""}`
						} : {
							ok: false,
							packageName: res.packageName,
							installedAsDisabled: res.installedAsDisabled === true,
		...
		async uninstall(name) {
			...
			if (isCoreComponent(n)) return {
				ok: false,
				message: `${n} 是 dsh Web GUI 的运行时组件,不能卸载(卸了界面就没了)。这是显示为"已安装"的系统组件,不是市场装的插件。`
			};
		...
		async listSessions() {
			try {
				...
				const headers = (await persistence.list()).map(listHeader).filter((h) => h !== void 0);
				...
				for (const h of headers) {
					const id = h.id;
					const live = Boolean(agents && agents.get(id) !== void 0 && agents.get(id).status === "running");
					let title = "";
					...
					sessions.push({
						...
						inWorkspace: workspaces.some((w) => w.sessionIds.includes(id))
					});
				}
		...
		*/
		async removeSessionFiles(persistence, header) {
			const targets = /* @__PURE__ */ new Set();
			try {
				const loc = persistence.locate(header);
				if (loc !== void 0 && loc.path) targets.add(dirname(loc.path));
			} catch {}
			try {
				const root = join(await this.getHome(), "sessions");
				if (existsSync(root)) for (const sub of readdirSync(root)) {
			...
			let failed;
			for (const dir of targets) try {
				rmSync(dir, {
					recursive: true,
					force: true
				});
			} catch (err) {
				failed = `删除会话文件失败:${err?.message || String(err)}`;
			}
			return failed;
		}
		/**
		...
		*/
		async forgetSessionCompat(registry, id) {
			if (typeof registry.forgetSession === "function") try {
		...
		async purgeSubagent(id, header) {
			...
			const persistence = this.persistenceFace;
			if (persistence !== void 0) await this.removeSessionFiles(persistence, header);
			const registry = this.workspaceRegistryFace;
		...
		*/
		...
		async setEnabled(name, enabled) {
			try {
				...
				if (isCoreComponent(n) && !enabled) return {
					ok: false,
					message: `${n} 是 dsh Web GUI 的运行时组件,不能停用(界面就没了)。`
				};
				const dir = await this.getProfileDir();
				this.invalidateListCache();
				const p = await this.readProfile();
		...
		* 装哪就是哪,用户装在哪不关我们事。自带没有时如实返回 false(让启动器去补齐)。 */
		async pnpmAvailable() {
			...
			cands.push(join(zt, "pnpm.exe"), join(zt, "pnpm.cjs"));
			for (const cand of cands) if (cand && existsSync(cand)) return true;
			return false;
		}
		async healthCheck() {
			const issues = [];
