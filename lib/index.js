			...
			try {
				...
				if (!s) {
					if ((await this.ghGet(`https://raw.githubusercontent.com/${o}/${r}/HEAD/package.json`)).status !== 200) {
						...
						if (sub.ok && sub.kind === "multi" && Array.isArray(sub.packages) && sub.packages.length > 0) return {
							ok: false,
							kind: "multi",
							packages: sub.packages,
							message: `${o}/${r} 是多插件仓库,请选择要安装的子插件。`
						};
						return {
							ok: false,
							message: `⚠ ${o}/${r} 是多插件仓库(根目录没有 package.json),且未找到可安装的子插件。请到该仓库的 GitHub 页面查看安装方式。`
						};
					}
				}
				...
				return res.ok ? {
					...
					message: `installed github:${o}/${r}${s ? `#path:${s}` : ""} — restart dsh to activate`
				} : {
					ok: false,
					message: res.message
				};
		...
		async update(owner, repo, subdir) {
			try {
				...
				return res.ok ? {
					ok: true,
					version,
					message: `updated github:${o}/${r}${s ? `#path:${s}` : ""} to v${version || "?"} — restart dsh to activate`
				} : {
					ok: false,
					message: res.message
				};
