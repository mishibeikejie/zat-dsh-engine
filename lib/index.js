		...
		async addSpec(owner, repo, subdir) {
			...
			if (pnpmResult.outcome.exitCode !== 0) {
				...
				return {
					ok: false,
					packageName: null,
					message: errText.slice(0, 2e3) || "pnpm failed"
				};
			}
				...
				const data = {
					...
					items: json.items.map((raw) => {
						...
						return {
							...
							isHarness: isHarness || void 0,
							disabled: rec && !rec.enabled ? true : void 0,
			...
			try {
				...
				if (!s) {
					if ((await this.ghGet(`https://raw.githubusercontent.com/${o}/${r}/HEAD/package.json`)).status !== 200) {
						...
						if (sub.ok && Array.isArray(sub.packages) && sub.packages.length > 0) {
							if (sub.packages.length === 1) {
								...
								return res.ok ? {
									...
									message: `已安装 ${only.name || o + "/" + r} — 重启 dsh 生效`
								} : {
									ok: false,
									message: res.message
								};
						...
						return {
							ok: false,
							message: "这个仓库没有找到可安装的插件,请到 GitHub 页面查看说明。"
						};
					}
				}
