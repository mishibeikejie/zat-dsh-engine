		...
		async checkMarketConflict(owner, repo) {
			...
			try {
				...
				for (const rec of Object.values(inst)) {
					...
					if (KNOWN_MARKET_REPOS[(rec.owner + "/" + rec.repo).toLowerCase()] !== void 0 || Object.values(KNOWN_MARKET_REPOS).includes(rec.name) || isMarketishName(rec.name)) conflicts.push(rec.name);
				}
		...
		*/
		async healthCheck() {
			...
			try {
				...
				const inst = this.installedMap(p);
				const markets = [];
				for (const rec of Object.values(inst)) if (KNOWN_MARKET_REPOS[(rec.owner + "/" + rec.repo).toLowerCase()] !== void 0 || Object.values(KNOWN_MARKET_REPOS).includes(rec.name) || isMarketishName(rec.name)) markets.push(rec.name);
