			...
			try {
				...
				for (const s of scanned) {
					...
					for (const f of scanSecurity(texts.hostText, `${s.name} 宿主代码`)) issues.push({
						level: f.level,
						title: f.title,
						detail: f.level === "error" ? "解决:点「一键修复」自动停用这个插件。" : f.detail,
						fixable: f.level === "error"
					});
		...
		async repair() {
			...
			try {
				...
				for (const name of deps) {
					...
					const texts = await this.readLocalTexts(name);
					if ([...scanSecurity(texts.hostText, name), ...scanSecurity(texts.clientText, name)].some((f) => f.level === "error")) secDisable.push(name);
				}
				if (secDisable.length > 0) {
					...
					p.dsh = p.dsh || {};
					p.dsh.profile = p.dsh.profile || {};
					p.dsh.profile.bundles = newBundles;
					await this.writeProfile(p);
					try {
						const checkBundles = JSON.parse(readFileSync(join(dir, "package.json"), "utf8")).dsh?.profile?.bundles;
