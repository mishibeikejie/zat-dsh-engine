...
- **装前体检 + 安全扫描** — 每个候选装之前先体检(入口文件、构建产物、依赖写法、网络去向);安全扫描抓混淆、偷凭据、可疑外发,❌ 硬伤直接拦截;AI 帮你找插件时也会如实报出体检结果
...
- **装前体检 + 安全扫描**:对每个候选和已装插件扫描结构、依赖、安全模式(混淆、偷凭据、可疑外发、系统修改),error 级直接拦截安装,网络去向装前全部列明。
...
- 新增「**AI 找插件**」:任意对话说出插件需求,AI 自动搜索市场、推荐候选,每个候选都附装前体检 + 安全扫描结论;带 ❌/⚠️ 的问题如实报出、不盲目推荐——不再"猛一看像那么回事,一用就翻车"
- 体检会揪出:入口文件缺失(dist 没提交、装完启动就报错)、挂载补丁缺失、官方包写成直接依赖、peer 依赖本机没装、安装脚本联网下载组件、代码依赖外部命令、仓库归档/停更
- 体检还会揪出"装完用不了"的隐藏坑:宿主代码引用没提交的构建产物(如 modlens 的 dist/main.js)直接标硬伤;依赖用户目录配置文件(如 ~/.modlens)标风险
- 安全扫描上线(装前体检 + 安装拦截 + 一键检测三层):混淆/动态执行、读取 SSH/云凭据/浏览器数据、可疑外发地址(机器人钩子、粘贴板、裸 IP、可疑域名)、修改注册表/计划任务/防火墙——error 级直接拒绝安装
- 网络去向透明化:每个候选装之前就列出它会连接哪些外部服务器,和功能对不上的一眼能看出来
- 搜索优先走 GitHub token(配额 5000/h)并缓存 10 分钟,模型连发查询不再触发 403 限流
- topic 搜索为空时自动放宽到全文搜索,给没打 dsh-plugin 标签的仓库一次机会(结果照常体检)
- 修复:本地版本领先 GitHub 时(开发版)不再误报"升级到旧版本";link 本地安装的开发版不提示更新,也不允许被 GitHub 版本覆盖
